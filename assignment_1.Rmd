...
> $\begin{align*}  `r paste0(paste0("t_{ ", names(tVector), " } & = "), paste0(round(tVector[names(tVector)], 4)), collapse="\\\\  ")`  
\end{align*}$  
...
print(paste("critical value:", rejectValue))
```
...
```{r plotting the power of the t-test}
## Creating muVect
...
plot(y = powerVect, x = muVect, xlab = expression(mu[X]), ylab = expression(Pi(mu[X]) ), type = "b", main = "Power of the t-test")
```
